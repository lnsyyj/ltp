...
#define TNAME "cmcc_posix/lstat-1.c"
...
int main(void)
{
    ...
    char *pathname = "/tmp/cmcc_posix_lstat_1";
    char *newpathname = "/tmp/cmcc_posix_lstat_1_new";
    struct stat info;
    ...
    result = lstat(newpathname, &info);
    if (result == -1)
    {
        printf(TNAME " Error at lstat(): %s\n", strerror(errno));
        return PTS_FAIL;
    }
